...
void getAllWorkersHandler(const drogon::HttpRequestPtr& request, callback_t&& callback)
{
    ...
    dbClient->execSqlAsync("SELECT id, name, surname, patronymic, role, storage_id, login, password FROM staff",
                           [callback](const drogon::orm::Result &result) {
                               if (!result.empty())
                               {
                                   ...
                                   sendResponse(SGCore::Serde::Serializer::toFormat(workers), drogon::HttpStatusCode::k200OK, callback, "value");
                               }
                           },
                           [callback](const drogon::orm::DrogonDbException& e) {
                               std::cerr << "Error selecting workers: " << e.base().what() << std::endl;
                               sendResponse("Error selecting workers!", drogon::HttpStatusCode::k400BadRequest, callback);
                           });
}
    ...
    dbClient->execSqlAsync("SELECT id, address FROM storages",
                           [callback](const drogon::orm::Result &result) {
                               if (!result.empty())
                               {
                                   ...
                                   std::cout << req << std::endl;
                                   sendResponse(SGCore::Serde::Serializer::toFormat(storages), drogon::HttpStatusCode::k200OK, callback, "value");
                               }
                           },
                           [callback](const drogon::orm::DrogonDbException& e) {
...
int main()
{
    ...
    drogon::app()
            .createDbClient("postgresql", "127.0.0.1", 5432, "warehouse", "postgres", "12345");
